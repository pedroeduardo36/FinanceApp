...
import { summarize, sumMoney, dateLabel } from '@/lib/finance';
...
export function PainelPage({ transacoes }: PainelPageProps) {
  ...
  const resumoMes = useMemo(() => {
    const report = summarize(transacoes, today.slice(0, 7) + '-01', today);
    ...
    return { receitas: report.totalReceitas, despesas: report.totalDespesas, saldoLiquido: report.saldoLiquido,
      graficoFormatado, ultimasTransacoes: [...transacoes].sort((a, b) => b.data_transacao.localeCompare(a.data_transacao)).slice(0, 5) };
  }, [transacoes, today]);
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-2">
        ...
        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm self-start md:self-auto">
          ...
          <span className="text-sm font-medium text-slate-600 capitalize">
            {format(parseISO(today), 'MMMM yyyy', { locale: ptBR })}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-sm font-medium text-slate-500">Saldo do mês até hoje</h3>
          </div>
