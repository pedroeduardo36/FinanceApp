import { cardTextColor } from '@/lib/colors';
...
export function CartoesPage({ userId, transacoes }: CartoesPageProps) {
  ...
  const { data: cartoes, loading, error: loadError, reload: fetchCartoes } = useRows('cartoes_credito', userId);
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    setSalvando(true);
    try {
      const limiteNum = moneyInput(limite, true);
      const payload = { user_id: userId, nome, ultimos_digitos: ultimosDigitos, limite: limiteNum, banco, cor, tipo };

      if (editandoId) {
        await requireMutation(supabase.from('cartoes_credito').update(payload).eq('id', editandoId).eq('user_id', userId).select());
      } else {
        await requireMutation(supabase.from('cartoes_credito').insert([payload]).select());
      }

      setIsModalOpen(false);
      await fetchCartoes();
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'Não foi possível salvar o cartão.');
    } finally {
      setSalvando(false);
    }
  };
  ...
  const handleExcluir = async (cartao: Cartao) => {
    try {
      ...
      await requireMutation(supabase.from('cartoes_credito').delete().eq('id', cartao.id).eq('user_id', userId).select());
      await fetchCartoes();

      setToast({ visible: true, cartao });
    } catch {
      setActionError('Erro ao excluir cartão.');
    }
  };
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <Feedback error={loadError} retry={() => void fetchCartoes()} />
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-lg font-bold text-slate-800">Meus Cartões</h2>
          <p className="text-sm text-slate-500">Gerencie limites e acompanhe suas faturas.</p>
        </div>
        <button
          onClick={() => abrirModal()}
          className="bg-emerald-700 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors shadow-sm"
        >
      ...
      {loading ? (
        ...
        <div className="bg-white p-8 rounded-xl border border-slate-200 text-center shadow-sm">
          ...
          <h3 className="text-slate-800 font-bold mb-2">Nenhum cartão cadastrado</h3>
            ...
            return (
              <div key={cartao.id} className="flex flex-col gap-4">
                <div
                  ...
                  style={{ backgroundColor: cartao.cor, color: cardTextColor(cartao.cor) }}
                >
                  <div className="absolute top-0 right-0 p-4  transition-opacity flex gap-2">
                    <button onClick={() => abrirModal(cartao)} className="p-1.5 bg-black/20 hover:bg-black/40 rounded-md backdrop-blur-sm transition-colors" aria-label="Editar">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleExcluir(cartao)} className="p-1.5 bg-black/20 hover:bg-red-500/80 rounded-md backdrop-blur-sm transition-colors" aria-label="Excluir">
                      <Trash2 size={16} />
                    </button>
                  </div>

                  <div className="flex justify-between items-start mb-8">
                    <span className="font-bold tracking-widest uppercase truncate max-w-[70%]">{cartao.banco || 'Banco'}</span>
                    <SmartphoneNfc size={24} className="opacity-80 shrink-0" />
                  </div>

                  <div className="mb-6">
                    <div className="text-sm uppercase tracking-widest mb-1">{cartao.tipo}</div>
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-[10px] uppercase tracking-wider mb-1">Titular do Cartão</div>
      ...
      {/* MODAL */}
      {isModalOpen && (
        <Dialog label="Cartão" onClose={() => setIsModalOpen(false)}>
          <Feedback error={actionError} />
          <form onSubmit={handleSalvar} className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="cartoespage-field-0" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Apelido</label>
                  <input id="cartoespage-field-0" type="text" required value={nome} onChange={(e) => setNome(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
                </div>
                <div>
                  <label htmlFor="cartoespage-field-1" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Banco / Emissor</label>
                  <input id="cartoespage-field-1" type="text" required value={banco} onChange={(e) => setBanco(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
                </div>
                <div>
                  <label htmlFor="cartoespage-field-2" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Últimos 4 Dígitos</label>
                  <input id="cartoespage-field-2" type="text" maxLength={4} value={ultimosDigitos} onChange={(e) => setUltimosDigitos(e.target.value.replace(/\D/g, ''))} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
                </div>
                <div>
                  <label htmlFor="cartoespage-field-3" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Limite (R$)</label>
                  <input id="cartoespage-field-3" type="number" min="0" step="0.01" required value={limite} onChange={(e) => setLimite(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
                </div>
              </div>

              <div>
                <label htmlFor="cartoespage-field-4" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Tipo</label>
                <select id="cartoespage-field-4" value={tipo} onChange={(e) => setTipo(e.target.value === 'debito' ? 'debito' : 'credito')} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none">
              ...
              <div>
                <p className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2">Cor do Cartão</p>
                <div className="flex flex-wrap gap-3">
                  {OPCOES_CORES.map((opcao) => (
                    <button
                      ...
                      type="button"
                      title={opcao.nome}
                      aria-label={opcao.nome} aria-pressed={cor === opcao.hex}
      ...
      {toast.visible && (
        <div role="status" aria-live="polite" className="fixed bottom-8 right-8 bg-slate-800 text-white px-5 py-3 rounded-xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-5 fade-in duration-300 z-50">
          ...
          <button onClick={() => setToast({ visible: false, cartao: null })} className="text-slate-600 hover:text-white transition-colors ml-2" aria-label="Fechar">
            <X size={16} />
          </button>
        </div>
      )}
