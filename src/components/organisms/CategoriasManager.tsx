...
export function CategoriasManager({ userId }: CategoriasManagerProps) {
  ...
  const handleExcluir = async (id: string) => {
    ...
    try {
      ...
      setActionError('Erro ao excluir categoria.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Feedback error={loadError} retry={() => void fetchCategorias()} />
      <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit">
          ...
          <form onSubmit={handleSalvar} className="space-y-4">
            ...
            <div>
              <label htmlFor="categoriasmanager-field-1" className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1">Subcategoria (Opcional)</label>
              <input id="categoriasmanager-field-1"
                type="text"
                value={subcategoria}
              ...
              {categorias.map(cat => (
                <div key={cat.id} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-lg group">
                  ...
                  <button
                    ...
                    className="text-slate-600 hover:text-red-500 p-1.5 rounded-md hover:bg-red-50 opacity-100  transition-all shrink-0"
